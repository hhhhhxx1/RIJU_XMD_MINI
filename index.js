...
function loadLocalFiles() {
  ...
  if (fs.existsSync('./lib/arslan.html')) {
    console.log("✅ riju.html found in lib folder");
  } else {
    console.log("⚠️ riju.html not found in lib folder");
  }
...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("RIJU_XMD_MINI~")) {
    console.log("❌ Invalid RIJU_XMD_MINI session format");
    process.exit(1);
  }
  ...
  console.log("♻️ RIJU MD session restored successfully");
}
    ...
    res.send(`
      ...
      <body>
        <div class="container">
          ...
          <p>Owner: ${config.OWNER_NAME || 'RIJU'}</p>
        </div>
      </body>
      </html>
    `);
...
async function connectToWA() {
  ...
  console.log(`🤖 RIJU MD Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ RIJU MD Bot connected to WhatsApp!");
      ...
      try {
        ...
        sock.sendMessage(ownerNumber[0] + '9173649345169@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
  ...
  sock.ev.on('messages.upsert', async (messageUpdate) => {
    try {
      ...
      if (msg.key && msg.key.remoteJid === 'status@broadcast') {
        ...
        if (config.AUTO_STATUS_REPLY === 'false' && msg.key.participant) {
          try {
      ...
      // ============ MODE HANDLING ============
      if (config.MODE === 'public' && isCmd && !isOwner) {
        return;
      }
      ...
      // ============ PRESENCE UPDATES ============
      if (config.AUTO_TYPING === 'false' && !msg.key.fromMe) {
...
.resetgoodbye`;
      ...
      // ============ AUTO RECORDING ============
      if (config.AUTO_RECORDING === 'false' && !msg.key.fromMe) {
